...
def get_cls_loss(pred, label, select):
    ...
    return F.nll_loss(pred, label, reduction='mean')
...
class DIOULoss(nn.Module):
    def forward(self, pred, target, weight=None):
        ...
        x_p = (pred_l + pred_r) / 2
        y_p = (pred_t + pred_b) / 2
        x_g = (target_l + target_r) / 2
        y_g = (target_t + target_b) / 2

        xc1 = torch.min(pred_l, target_l)
        yc1 = torch.min(pred_t, target_t)
        xc2 = torch.max(pred_r, target_r)
        yc2 = torch.max(pred_b, target_b)
        ...
        covered = ((xc2 - xc1) ** 2) + ((yc2 - yc1) ** 2) +1e-7
        dist = ((x_p - x_g) ** 2) + ((y_p - y_g) ** 2)
        R_u = dist / covered

        iou = (area_intersect/area_union) - R_u
        losses = 1-iou
        return losses.mean()
...
class CIOULoss(nn.Module):
    def forward(self, pred, target, weight=None):
        ...
        w_pred = pred_r - pred_l
        h_pred = pred_b - pred_t
        w_gt = target_r - target_l
        h_gt = target_b - target_t
